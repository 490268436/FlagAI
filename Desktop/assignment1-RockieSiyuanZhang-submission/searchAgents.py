...
class BidirectionalFoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        self.start = [self.init_pos] + self.getGoalStates()
    ...
    def getStartState(self):
        ...
        # print(self.start)
        self.found+=1
        self.goal = self.goals[self.found-1]
        return self.start[self.found-1]
    ...
    def getGoalStates(self):
        ...
        all_goals =[]
        for i in range(self.foodGrid.width):
            for j in range(self.foodGrid.height):
                if self.foodGrid[i][j] ==True:
                    all_goals.append((i,j))
        min_distance =float('inf')
        start = self.init_pos
        first_goal =None
        for goal in all_goals:
            if mazeDistance(start , goal,self.startingGameState)<min_distance:
                min_distance = mazeDistance(start , goal,self.startingGameState)
                first_goal = goal
        
        goal_states.append(first_goal)
        all_goals.remove(first_goal)

        ite_times = len(all_goals)
        pre_goal = first_goal
        for i in range(ite_times):
            next_ng = None
            mindis_next = float('inf')
            for ng in all_goals:
                if mazeDistance(pre_goal, ng,self.startingGameState)<mindis_next:
                    mindis_next = mazeDistance(first_goal , ng,self.startingGameState)
                    next_ng = ng
            goal_states.append(next_ng)
            all_goals.remove(next_ng)
            pre_goal = next_ng
    ...
    def getBackwardsSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_x][next_y]:
                ...
                successors.append((nextState, rev_action,1))             
...
def bidirectionalFoodProblemHeuristic(state, problem):
    ...
    assert not walls[x1][y1], 'point1 is a wall: ' + str(state)
    assert not walls[x2][y2], 'point2 is a wall: ' + str(problem.goal)
    prob = PositionSearchProblem(problem.startingGameState, start=state, goal=problem.goal, warn=False, visualize=False)
    return len(search.astar(prob))


def bidirectionalFoodProblemBackwardsHeuristic(state, problem):
    "*** YOUR CODE HERE for Task 3 ***"
    # return util.manhattanDistance(state,problem.start[problem.found])
    x1, y1 = state
    x2, y2 = problem.start[problem.found]
    walls = problem.startingGameState.getWalls()
    assert not walls[x1][y1], 'point1 is a wall: ' + str(state)
    assert not walls[x2][y2], 'point2 is a wall: ' + str(problem.start[problem.found])
    prob = PositionSearchProblem(problem.startingGameState, start=state, goal=problem.start[problem.found], warn=False, visualize=False)
    return len(search.astar(prob))
